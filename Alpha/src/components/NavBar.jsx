...
const Navbar = React.forwardRef((props, ref) => {
  ...
  return (
    <section className=" flex min-h-screen bg-indigo-800 rounded-xl w-auto ml-0">
      <Sidebar navItems={navItems} />
      <X
        size={30}
        ref={ref}
        onClick={props.toggleNavbar}
        className="text-white font-bold mt-4 mr-7"
      />
    </section>
  );
});
...
const Sidebar = ({ navItems }) => {
  return (
    <div className="relative inset-y-0 left-0 md:w-auto text-white pt-7 flex flex-col">
      {/* Logo Section */}
      ...
      <div className=" mt-48 md:mt-56 ml-5 mb-5 ">
        <UserCard />
      </div>
    </div>
  );
...
const NavItem = ({ icon, text }) => {
  return (
    <li>
      <button className="w-full flex items-center gap-3 px-4 py-3 rounded-lg text-white/80 hover:text-white hover:bg-white/10 transition-colors duration-200">
